pipeline {
    ...
    stages {
        ...
        stage('docker build') {
            steps {
                sh(script: """
                    docker images -a
                    docker build -t gradproj/5g-ausf:latest . 
                    docker images -a
                """)
            }
        }
        ...
        stage('Scan Image for Common Vulnerabilities and Exposures') {
            steps {
                sh 'trivy image gradproj/5g-ausf --output trivy-report.json'
            }
        }
        stage('Pushing to Dockerhub') {
            steps {
                sh 'docker push gradproj/5g-ausf:latest'
            }
        }
    ...
    post {
        always {
            ...
            sh 'docker rmi gradproj/5g-ausf'
            sh 'docker rmi gradproj/ausf-base'
        }
    }
}
